...
async def oura_authorize_endpoint() -> RedirectResponse:
    if not get_settings().oura_client_id:
        raise HTTPException(
            status_code=503, detail="Oura client credentials are not configured"
        )
    return RedirectResponse(build_authorize_url())


@router.get("/oura/callback")
async def oura_callback_endpoint(
    code: str,
    state: str,
    session: AsyncSession = Depends(get_session),
    user_id: UUID = Depends(get_current_user_id),
    ...
    try:
        await exchange_oura_code(session, user_id, code)
    except httpx.HTTPError as e:
        # No token material in the detail — just the failure class
        raise HTTPException(
            status_code=502, detail="Oura token exchange failed"
        ) from e
    return RedirectResponse("http://localhost:3000/settings?connected=oura")
