...
from app.models import ImportJob, OAuthConnection, SleepRecord, User
...
async def _add_connection(
    session: AsyncSession, user: User, *, expires_in_seconds: int
) -> OAuthConnection:
...
async def test_oura_sync_maps_fields_and_is_idempotent(
    client: AsyncClient, session: AsyncSession, oura_creds, isolated_user
) -> None:
    # Valid token, so any hit on the (unmocked) token endpoint would error
    _mock_collections()
    await _add_connection(session, isolated_user, expires_in_seconds=3600)

    job_id = await _trigger_sync(client)

    records = await _fetch_records(session, isolated_user)
...
async def test_oura_sync_refreshes_expired_token(
    ...
    connection = await _add_connection(session, isolated_user, expires_in_seconds=-60)
    ...
    job = await session.get(ImportJob, job_id)
    assert job.status.value == "completed"
    assert len(await _fetch_records(session, isolated_user)) == 2
...
async def test_oura_sync_without_connection_fails_job(
    ...
    assert await _fetch_records(session, isolated_user) == {}


async def test_oura_sync_without_credentials_fails_job(
    client: AsyncClient, session: AsyncSession, monkeypatch, isolated_user
) -> None:
...
async def test_oura_oauth_flow(
    ...
    result = await session.execute(
        select(OAuthConnection).where(
            OAuthConnection.user_id == isolated_user.id,
